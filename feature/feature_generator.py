...
class FeatureGenerator:
    def __init__(self, individual_window_size, head_to_head_window_size):
        ...
        self._match_results_df.sort_values(by=['date'], inplace=True)
        self._rankings_df.sort_values(by=['rank_date'], inplace=True)
    ...
    def _process_row(self, row):
        ...
        features = self.get_features(home_team, away_team, match_date, match_type, neutral)
    ...
    def _get_average_opposition_rank_diff(self, team, date_of_new_match, match_type):
        ...
        oppositions = []
        for index, row in team_a_matches.iterrows():
            if row['home_team'] == team:
                oppositions.append([row['away_team'], row['date']])
            else:
                oppositions.append([row['home_team'], row['date']])
        for opposition in oppositions:
            team_rank, team_rank_change = self._get_closest_ranking_for_team(team, opposition[1])
            opposition_rank, opposition_rank_change = self._get_closest_ranking_for_team(opposition[0], opposition[1])
            opposition_rank_diff.append(opposition_rank - team_rank)
    ...
    def _get_head_to_head_statistics(self, home_team, away_team, date_of_new_match, match_type):
        ...
        if head_to_head_matches.empty:
            return 0, 0, 0, 0
        home_goals_scored = np.concatenate(
            (head_to_head_matches[head_to_head_matches['home_team'] == home_team]['home_score'].to_numpy(),
             head_to_head_matches[head_to_head_matches['away_team'] == home_team]['away_score'].to_numpy()))
        away_goals_scored = np.concatenate(
            (head_to_head_matches[head_to_head_matches['home_team'] == away_team]['home_score'].to_numpy(),
             head_to_head_matches[head_to_head_matches['away_team'] == away_team]['away_score'].to_numpy()))
        home_mean_goals_scored = np.mean(home_goals_scored)
        away_mean_goals_scored = np.mean(away_goals_scored)
        home_std_goals_scored = np.std(home_goals_scored)
        away_std_goals_scored = np.std(away_goals_scored)
        return home_mean_goals_scored, away_mean_goals_scored, home_std_goals_scored, away_std_goals_scored
    ...
    def _get_match_type(self, match_type):
        if match_type == 'Friendly':
            return 1, 0, 0
        elif 'qualification' in match_type.lower():
            return 0, 1, 0
        else:
            return 0, 1, 1
